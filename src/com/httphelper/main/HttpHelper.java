...
public class HttpHelper {
  ...
  private HttpHelper(String host, int port, HttpVersion defaultVersion, String defaultHost,
      boolean keepAlive, double minDelay, double maxDelay, List<Header> defaultHeaders,
      Set<Cookie> cookies, boolean useHttps) {
    super();
    this.host = host;
    this.port = port;
  ...
  public HttpRequest get(String path) {
    HttpRequest req = new HttpGetRequest(this).setVersion(defaultVersion).setPath(path)
        .addHeader(Headers.host(defaultHost)).addHeaders(defaultHeaders);
    if (!cookies.isEmpty()) {
  ...
  public HttpRequest post(String path) {
    HttpRequest req = new HttpPostRequest(this).setVersion(defaultVersion).setPath(path)
        .addHeader(Headers.host(defaultHost)).addHeaders(defaultHeaders);
    if (!cookies.isEmpty()) {
      req.addHeader(Headers.cookie(validCookies()));
    }
  ...
  public synchronized HttpResponse send(HttpRequest httpRequest) {
    delay();
    try {
      if (useHttps) {
  ...
  private void sendHttps(HttpRequest httpRequest) throws IOException {
    httpConnection =
        (HttpURLConnection) new URL("https://" + host + httpRequest.getPath()).openConnection();
  ...
  private HttpResponse waitResponse(HttpRequest req) {
    if (socket.isClosed()) {
    ...
    HttpResponse response =
        new HttpResponse(this, responseStr.toString(), Arrays.copyOf(byteInfo.arr, byteInfo.size));
  ...
  private void appendHttpsHeaders(ByteInfo byteInfo, StringBuilder responseStr) throws IOException {
    ...
    httpConnection.getHeaderFields().entrySet().stream().filter(e -> e.getKey() != null).forEach(
        e -> e.getValue().forEach(v -> responseStr.append(e.getKey() + ": " + v + "\r\n")));
    responseStr.append("\r\n");
  ...
  private synchronized void handleCookies(HttpResponse response) {
  ...
  public HttpHelper clone(String host, int port) {
    return new HttpHelper(host, port, defaultVersion, defaultHost, keepAlive, minDelay, maxDelay,
        new ArrayList<>(defaultHeaders), cookies, useHttps);
  }
